...
def run_tray_app() -> None:
    ...
    state = _TrayState(status=get_tray_status(), error="")

    def refresh(icon: object | None = None) -> None:
        state["status"] = get_tray_status()
        status = state["status"]
        if icon is not None:
            tray_icon = cast("_TrayIcon", icon)
            tray_icon.title = _tooltip(status, state["error"])
            tray_icon.icon = _build_icon(Image, ImageDraw, status.running)
